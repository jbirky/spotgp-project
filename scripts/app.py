...
if has_raw:
    ...
    downsample = st.sidebar.number_input(
        "Downsample (every Nth point)", min_value=1,
        max_value=n_raw, value=1, step=1)
    ...
    st.session_state["data_arrays"] = process_data(
        segments, downsample, normalize, zero_mean)
...
if envelope_name == "TrapezoidSymmetricEnvelope":
    envelope_params["lspot"] = st.sidebar.slider(
        "lspot (days)", 0.0, 100.0, 15.0, 0.5)
    envelope_params["tau_spot"] = st.sidebar.slider(
        "tau_spot (days)", 0.1, 50.0, 5.0, 0.1)
elif envelope_name == "TrapezoidAsymmetricEnvelope":
    envelope_params["lspot"] = st.sidebar.slider(
        "lspot (days)", 0.0, 100.0, 15.0, 0.5)
    envelope_params["tau_em"] = st.sidebar.slider(
        "tau_em (days)", 0.1, 50.0, 5.0, 0.1)
    envelope_params["tau_dec"] = st.sidebar.slider(
        "tau_dec (days)", 0.1, 50.0, 5.0, 0.1)
    ...
    envelope_params["sigma_sn"] = st.sidebar.slider(
        "sigma_sn (days)", 0.1, 50.0, 5.0, 0.1)
    envelope_params["n_sn"] = st.sidebar.slider(
        "n_sn", 0.0, 10.0, 2.0, 0.1)
...
visibility_params = {}
visibility_params["peq"] = st.sidebar.slider(
    "P_eq (days)", 0.01, 40.0, 10.0, 0.01)
if visibility_name != "EdgeOnVisibilityFunction":
    visibility_params["kappa"] = st.sidebar.slider(
        "kappa", -1.0, 1.0, 0.3, 0.01)
    visibility_params["inc"] = st.sidebar.slider(
        "inc (deg)", 0.0, 90.0, 60.0, 1.0) * np.pi / 180.0
...
if latitude_name == "UniformDoubleHemisphereBand":
    latitude_params["min_lat_deg"] = st.sidebar.slider(
        "Min latitude (deg)", 0.0, 89.0, 0.0, 1.0)
    latitude_params["max_lat_deg"] = st.sidebar.slider(
        "Max latitude (deg)", 1.0, 90.0, 40.0, 1.0)
elif latitude_name == "ButterflyLatitude":
    latitude_params["phi0_deg"] = st.sidebar.slider(
        "phi0 (deg)", 0.0, 60.0, 15.0, 1.0)
    latitude_params["sigma_deg"] = st.sidebar.slider(
        "sigma_phi (deg)", 1.0, 30.0, 7.0, 0.5)

st.sidebar.markdown("---")

log_sigma_k = st.sidebar.slider("log sigma_k", -6.0, 0.0, -2.0, 0.1)
sigma_k = 10.0 ** log_sigma_k
...
if col_fit.button("Predict", use_container_width=True):
    if "model_params" not in st.session_state:
        ...
        try:
            ...
            _gp = _sg.GPSolver(_data, _mdl).build_jax()
...
st.title("spotgp Explorer" + (f" — {star_label}" if star_label else ""))
...
if not model_ready:
    ...
    for n in _selected_harmonics:
        Kn = harmonics[n]
        if _normalize_kernel:
            Kn = Kn / _K0_manual
        fig_kernel.add_trace(go.Scatter(
            x=lag, y=Kn, mode="lines",
            name=f"n={n}",
            line=dict(width=1, color=_harmonic_colors[n], dash="dash"),
            opacity=0.7,
            hovertemplate=f"n={n}<br>Lag: %{{x:.2f}}<br>K: %{{y:.4f}}"
                          "<extra></extra>"))

    if "map_kernel_data" in st.session_state:
        map_lag, map_K = st.session_state["map_kernel_data"]
        map_K_plot = (map_K / map_K[0] if map_K[0] != 0 else map_K
                      ) if _normalize_kernel else map_K
        fig_kernel.add_trace(go.Scatter(
            x=map_lag, y=map_K_plot, mode="lines",
            name="MAP kernel",
...
st.markdown("---")
st.subheader("MAP Optimization")

if not has_data:
    st.info("Load data to enable fitting.")
elif not model_ready:
    st.info("Generate a model to enable fitting.")
elif kernel_error is not None:
    st.error("Fix model errors before fitting.")
else:
    col_fit_left, col_fit_right = st.columns([1, 2])

    with col_fit_left:
        nopt = st.number_input("Number of restarts", min_value=1,
                               max_value=50, value=5)

        _candidates = {}
        _candidates.update(mp["visibility_params"])
        _candidates.update(mp["envelope_params"])
        _candidates.update(mp.get("latitude_params", {}))
        _candidates["log_sigma_k"] = mp["log_sigma_k"]

        fit_keys = st.multiselect(
            "Parameters to fit", list(_candidates),
            default=[k for k in ("peq", "kappa", "log_sigma_k")
                     if k in _candidates])

        fit_bounds = {}
        for _key in fit_keys:
            _lo_def, _hi_def = _default_fit_bounds(_key, _candidates[_key])
            _c_lo, _c_hi = st.columns(2)
        ...
        default_save = (f"results/{star_label.replace(' ', '_')}.h5"
                        if star_label else "results/fit.h5")
        save_path = st.text_input("Save path", value=default_save)

        run_fit = st.button("Run MAP Fit", type="primary")
    ...
    if run_fit:
        ...
        envelope_cls = getattr(spotgp, mp["envelope_name"])
        envelope = envelope_cls(**mp["envelope_params"])
        vis_cls = getattr(spotgp, mp["visibility_name"])
        ...
        model = spotgp.SpotEvolutionModel(
            ...
            sigma_k=mp["sigma_k"], latitude_distribution=fit_lat)

        with st.spinner("Building GP solver..."):
            gp = spotgp.GPSolver(data_obj, model,
                                 bounds=fit_bounds).build_jax()

        with st.spinner(f"Running MAP optimization ({nopt} restarts)..."):
            theta_map, result = gp.fit_map(nopt=nopt,
                                           keys=list(fit_keys) or None)
        ...
        _map_lat = _build_latitude_dist(
            map_mp.get("latitude_name", "LatitudeDistributionFunction"),
            map_mp.get("latitude_params", {}))
        _map_model = spotgp.SpotEvolutionModel(
            envelope=_map_env, visibility=_map_vis,
            sigma_k=map_mp["sigma_k"], latitude_distribution=_map_lat)
        ...
        st.session_state["map_kernel_data"] = (_map_lag, _map_K)
    ...
    if "fit_result" in st.session_state:
        ...
        with col_fit_right:
            ...
            if isinstance(theta_map, dict):
                st.table({k: f"{v:.6f}" for k, v in theta_map.items()})
            else:
                st.write(theta_map)

        col_pred, col_acf_fit = st.columns(2)

        with col_pred:
            st.subheader("GP Prediction")
            try:
                fig_pred = gp.plot_prediction()
        ...
        with col_acf_fit:
            st.subheader("Fitted ACF")
            try:
                fig_acf_fit = gp.plot_acf()
                if fig_acf_fit is not None:
                    st.pyplot(fig_acf_fit)
                    plt.close(fig_acf_fit)
            except Exception as e:
        ...
        if st.button("Save results"):
            ...
            with h5py.File(save_path, "w") as f:
                ...
                mg.attrs["envelope"] = mp["envelope_name"]
                mg.attrs["visibility"] = mp["visibility_name"]
                mg.attrs["sigma_k"] = mp["sigma_k"]
                for k, v in mp["envelope_params"].items():
                    mg.attrs[f"envelope_{k}"] = v
                for k, v in mp["visibility_params"].items():
